...
function getCachedUserProfile(userId: number) {
    const cachedOperation = nextCache(getUserProfile, [`user-profile-${userId}`], { tags: [`user-profile-${userId}`] });
...
export default async function ChatRoom({ params }: { params: Promise<{ id?: string }> }) {
    ...
    if (!chatRoomId) {
        return <ErrorPage title="잘못된 접근" message="유효한 채팅방 ID가 없습니다." />;
    }
    // 비동기 작업 병렬 실행
    ...
    if (!session?.id) {
        return <ErrorPage title="세션 오류" message="로그인이 필요합니다." />;
    }
    if (!room) {
        return <ErrorPage title="채팅방을 찾을 수 없습니다." message="존재하지 않거나 접근 권한이 없는 채팅방입니다." />;
    }
    ...
    const onSold = async () => {
        ...
        revalidateTag(`user-profile-${users.filter((user) => user.id !== session.id)[0]}`);
